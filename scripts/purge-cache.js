...
async function purgeCache(packageName, version) {
  ...
  let urls = files.map(
    file => `https://npm.onmicrosoft.cn/${packageName}@${version}${file.path}`
  );

  if (version === 'latest') {
    // Purge the URL w/out the "@latest" too.
    urls = urls.concat(
      files.map(file => `https://npm.onmicrosoft.cn/${packageName}${file.path}`)
    );
  }

  return getZone('npm.onmicrosoft.cn').then(zone => {
